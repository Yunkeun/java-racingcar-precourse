...
## 리팩토링 목록
- [X] 객체에서 데이터를 꺼내지 않고 객체에 메시지 전달하기
- [X] 인스턴스 변수 줄이기
- [X] 비즈니스 로직과 UI 로직 분리하기
  - controller에서 UI 로직 사용하기 (MVC 패턴에 의거)
- [X] 최대한 side effect 없게 구현하기
...
### 게임 진행
- [X] 각 자동차에 대해 0에서 9사이의 random 값을 구한다.
  - random 값이 4 이상인 경우 전진
- [X] 자동차의 위치는 0에서 시작하여 전진인 경우 1씩 증가한다.
  - 위치에 대한 출력으로 '-'를 사용한다.

### 종료
- [X] 유저가 입력한 시도 횟수만큼 진행한다면 프로그램을 종료한다.
  - [X] 제일 앞선 자동차가 우승자가 되어 우승자 안내 문구를 출력한다. 단, 우승자는 한 명 이상일 수 있다.
